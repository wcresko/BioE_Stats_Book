...
- **Small k** (e.g., k=1): Very flexible decision boundary, low bias but high variance. The boundary follows noise in the training data—prone to overfitting.
...
The **AUC** (Area Under the ROC Curve) summarizes classifier performance:
...
Methods like KNN face a challenge when many predictors are used, known as the **curse of dimensionality**. In high-dimensional spaces:

- All points become approximately equidistant
- "Local" neighborhoods must span nearly the entire data
...
- **K-Nearest Neighbors (KNN)** classifies based on the majority class of nearby training points
  - Choice of k represents a bias-variance tradeoff
  - Small k: flexible but noisy; large k: smooth but may miss local patterns
  - Cross-validation should be used to select optimal k
...
- **Confusion matrices** summarize classification performance:
  ...
  - F1 score and balanced accuracy are better for imbalanced data
...
- The **curse of dimensionality** limits KNN in high-dimensional spaces
