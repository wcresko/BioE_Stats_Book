...
The best predictions come from models that balance bias and variance. As model complexity increases:

- **Bias decreases**: More complex models can fit more patterns
- **Variance increases**: More complex models are more sensitive to training data
- **Total error**: Initially decreases (bias reduction dominates), then increases (variance inflation dominates)
...
**Tradeoffs:**
- LOOCV has lower bias (nearly all data used for training) but higher variance
- 5-fold or 10-fold CV provides a good balance of bias and variance
- LOOCV is computationally expensive for large datasets

### Bootstrap for Error Estimation

The **bootstrap** can also estimate prediction error:
...
A typical model validation workflow:

1. **Split data** into training set (e.g., 80%) and test set (20%)
2. **Explore** the training data—never look at the test set!
3. **Use cross-validation** on the training set to:
   - Compare different model types
   - Tune hyperparameters
   - Estimate expected performance
4. **Select final model** based on CV performance
5. **Refit** on full training data
6. **Evaluate once** on held-out test data
7. **Report** the test error as the honest estimate of performance
...
- **Overfitting** occurs when models fit training noise rather than true patterns
- **Loss functions** quantify prediction error—choice should match application goals
- **Training error** underestimates true prediction error; always evaluate on held-out data
- The **bias-variance tradeoff** governs model complexity:
  - Simple models: high bias, low variance
  - Complex models: low bias, high variance
  - Optimal models balance both
- **Cross-validation** estimates generalization error without wasting data:
  - K-fold (typically k=5 or 10) balances bias and variance
  - LOOCV has low bias but high variance and computational cost
  - Bootstrap provides alternative estimates with confidence intervals
- The **one standard error rule** guards against overfitting in model selection
- **Information criteria** (AIC, BIC) provide alternatives to CV for model comparison
- A disciplined workflow separates training, validation, and testing phases
