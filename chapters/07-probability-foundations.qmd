...
In statistics, we distinguish between **populations** and **samples**:

- A **population** is the complete collection of all individuals or observations of interest
- A **sample** is a subset drawn from the population

We rarely observe entire populations. Instead, we take samples and use them to estimate population parameters. The mean of our sample ($\bar{x}$) estimates the true population mean ($\mu$). The sample variance ($s^2$) estimates the population variance ($\sigma^2$).

The process of statistical inference involves using sample statistics to draw conclusions about population parameters—while quantifying the uncertainty in those conclusions.
...
We can estimate the Bernoulli distribution empirically by flipping a coin many times:
...
The observed proportions are very close to the theoretical probability of 0.5 for a fair coin.
