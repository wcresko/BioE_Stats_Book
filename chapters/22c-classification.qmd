...
- **No training phase**: All computation happens at prediction time ("lazy learning")
- **Non-parametric**: Makes no assumptions about data distribution
- **Naturally handles multiclass problems**: No modification needed
- **Captures complex patterns**: Can model non-linear decision boundaries

### Limitations

- **Computationally expensive for large datasets**: Must compute distances to all training points for each prediction
- **Sensitive to irrelevant features**: All features contribute equally to distance
- **Requires feature scaling**: Variables on different scales bias the results
- **Curse of dimensionality**: In high dimensions, all points become approximately equidistant
- **No model to interpret**: Cannot identify which features are most important
